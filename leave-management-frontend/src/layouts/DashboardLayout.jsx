...
const DashboardLayout = ({ children, unreadCount, notifications }) => {
    ...
    // Dropdown Navigation Menus States
    const [openTimeMgmt, setOpenTimeMgmt] = useState(true);
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex font-sans antialiased">
            ...
            <aside className="w-72 bg-[#0082c3] text-white flex flex-col fixed inset-y-0 left-0 z-40 shadow-xl select-none">
                ...
                {/* Application Branding / Logo Header Wrapper */}
                ...
                <nav className="flex-1 px-4 py-6 overflow-y-auto space-y-1 scrollbar-thin">
                    ...
                    <a href="/dashboard" className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/10 font-medium text-sm transition">
                    ...
                    {user?.role === "supervisor" && (
                        <a href="/employees" className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 font-medium text-sm text-sky-100 hover:text-white transition group">
                            <svg className="w-5 h-5 text-sky-200 group-hover:text-white transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="space-y-1">
                        ...
                        {openTimeMgmt && (
                            <div className="pl-11 pr-2 space-y-1 border-l-2 border-white/10 ml-5 animate-slideDown">
                                <a href="/holidays" className="block py-2 px-3 text-xs font-medium text-sky-100 hover:text-white rounded-lg hover:bg-white/5 transition">
                                    Company Holidays
                                </a>
                            </div>
                        )}
                    ...
                    {/* DROPDOWN 2: REQUESTS SUB-MENU MAPPING */}
                    <div className="space-y-1">
                        <button 
                        ...
                        {openRequests && (
                            <div className="pl-11 pr-2 space-y-1 border-l-2 border-white/10 ml-5">
                                <a href="/dashboard" className="flex items-center gap-2 py-2 px-3 text-xs font-semibold text-white bg-white/10 rounded-lg transition relative">
                                    <span className="w-1.5 h-1.5 bg-white rounded-full absolute left-0"></span>
                                    Time requests
                                </a>
                                <a href="/mission-orders" className="block py-2 px-3 text-xs font-medium text-sky-100 hover:text-white rounded-lg hover:bg-white/5 transition">
                                    Mission orders
                                </a>
                                <a href="/expenses" className="block py-2 px-3 text-xs font-medium text-sky-100 hover:text-white rounded-lg hover:bg-white/5 transition">
                                    Expenses
                                </a>
                            </div>
                        )}
            ...
            <div className="flex-1 pl-72 flex flex-col min-h-screen">
                ...
                {/* Persistent Top Navbar Layer Component */}
                <header className="h-16 bg-white border-b border-gray-100 px-8 flex items-center justify-between shadow-sm sticky top-0 z-30">
                    <div>
                        <h1 className="text-xl font-bold text-gray-800 tracking-tight">Time requests</h1>
                    </div>

                    <div className="flex items-center gap-5">
                        {/* Notification Bell Structure Dropdown Entry Point */}
                        ...
                        {/* User Header Profile Identity Widget */}
                        <div className="flex items-center gap-3">
                ...
                {/* Inner Core Page Render Output Context */}
